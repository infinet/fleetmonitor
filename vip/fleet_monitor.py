...
class FleetMonitor(Tiger):
    ...
    def get_vipkey(self, vessel_name):
        ...
        # to request shared aes key sets generated by vessel, vip send a
        # message contains req_shrvessel_name, gapp will test for this name
        dbreq = '{0:20}'.format('reqshr-' + vessel_name)
        ...
        payload = (obfus_key +
                   self.encrypt_aes(dbreq,
                                    aeskey=self.session_key,
                                    hmackey=self.session_hmac_key))
        # post to gapp
        e_obj = open_request(self.fetch_srv, payload).read()
        shared_vipkey = self.decrypt_aes(e_obj,
                                           aeskey=self.session_key,
                                           hmackey=self.session_hmac_key)
    ...
    def get_vessel_location(self):
        ...
        # reqvesselgps is the token for ask gps data
        msg = '{0:20}'.format('reqvesselgps')
        ...
        payload = self.decrypt_aes(e_obj,
                                aeskey=self.keysoup['s_key'],
                                hmackey=self.keysoup['s_hmac_key'])
